...
uses
  Windows, SysUtils,
  UtilsB2, Concur, DataLib, StrLib,
  VfsUtils, VfsBase, VfsMatching, Legacy;
type
  (* Import *)
...
function GetOpenedFilePath (hFile: Windows.THandle): myWStr;
...
constructor TOpenedFile.Create (hFile: Windows.THandle; const AbsPath: myWStr);
begin
  Self.hFile   := hFile;
...
begin
  Legacy.FreeAndNil(Self.DirListing);
end;

procedure TOpenedFile.FillDirListing (const Mask: myWStr);
var
...
begin
  ...
  ExcludedItems   := DataLib.NewDict(not UtilsB2.OWNS_ITEMS, DataLib.CASE_SENSITIVE);
  ...
  // * * * * * //
  Legacy.FreeAndNil(ExcludedItems);
...
begin
  ...
  OpenedFiles := DataLib.NewObjDict(UtilsB2.OWNS_ITEMS);
end.