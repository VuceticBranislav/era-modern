...
uses
  ...
  UtilsB2, Concur, WinUtils, StrLib, WinNative,
  VfsBase, VfsUtils, Legacy;
...
function RunWatcher (const WatchDir: myWStr; DebounceInterval: integer): boolean;
...
type
  ...
  TDirChangesIterator = class (UtilsB2.TManagedObject, IDirChangesIterator)
   protected const
     BUF_SIZE = 65500;
   ...
   public
    constructor Create (const DirPath: myWStr); overload;
...
constructor TDirChangesIterator.Create (const DirPath: myWStr);
const
  MANUAL_RESET_EVENT = true;

begin
  Self.fDirPath   := VfsUtils.NormalizePath(DirPath);
  Self.fDirHandle := Windows.CreateFileW(myPWChar(Self.fDirPath), Windows.GENERIC_READ, Windows.FILE_SHARE_READ or Windows.FILE_SHARE_WRITE, nil,
...
begin
  ...
  if Self.fBufPos < fBufSize then begin
    ...
    Self.fBufPos := UtilsB2.IfThen(NotifInfoInBuf.NextEntryOffset <> 0, Self.fBufPos + integer(NotifInfoInBuf.NextEntryOffset), Self.BUF_SIZE);
  end else begin
    Legacy.FillChar(AsyncRes, sizeof(AsyncRes), 0);
        ...
        WinUtils.WR_WAITED: begin
          if TriggeredEvent = StopEvent then begin
            ...
            if result then begin
              ...
              end else if Self.fBufSize < sizeof(NotifInfoInBuf^) + sizeof(myWChar) then begin
                result := false;
              end else begin
...
function ReadDirChanges (const DirPath: myWStr): IDirChangesIterator;
begin
  ...
  with VfsBase.GetThreadVfsDisabler do begin
    ...
    try
      while not IsEnd do begin
        ...
        if not DirChangesScanner.IterNext(DirChange, WatcherStopEvent, UtilsB2.IfThen(boolean(NeedFullRescan), integer(PlannedRescanTime - CurrentTime), integer(Windows.INFINITE))) then begin
          // Force scanner recreation later
          DirChangesScanner := nil;

          // Wait and retry, unless stop signal is received
          Timeout := UtilsB2.IfThen(NeedFullRescan, Min(WatcherDebounceInterval, integer(PlannedRescanTime - CurrentTime)), WatcherDebounceInterval);
