...
begin
  ...
  if (Kernel32Handle <> 0) and (KernelBaseHandle <> 0) then begin
    NormalProc := Windows.GetProcAddress(Kernel32Handle, myPChar('GetCurrentProcessId'));
    RealProc   := Windows.GetProcAddress(KernelBaseHandle, myPChar('GetCurrentProcessId'));
