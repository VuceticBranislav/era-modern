...
uses
  ...
  UtilsB2, WinUtils,
  VfsUtils, VfsBase, VfsTestHelper, Legacy;
...
begin
  ...
  DirListing.Rewind;
  CheckEquals('', string(DirListing.GetDebugDump()), 'Virtual directory listing must be empty when VFS is paused');

  VfsBase.RunVfs(SORT_FIFO);
  VfsBase.GetVfsDirInfo(RootDir, '*', DirInfo, DirListing);
  DirListing.Rewind;
  CheckEquals('vcredist.bmp'#13#10'eula.1028.txt', string(DirListing.GetDebugDump()), 'Invalid virtual directoring listing' );
