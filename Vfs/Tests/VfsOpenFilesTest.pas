...
begin
  ...
  CheckEquals(VALID_FULLY_VIRT_DIR_LISTING, string(OpenedFile.DirListing.GetDebugDump()), 'Invalid listing for fully virtual directory "' + DirPath + '"');
  FreeAndNil(OpenedFile);

  OpenedFile := VfsOpenFiles.TOpenedFile.Create(888, RootDir);
  OpenedFile.FillDirListing('*');
  Check(OpenedFile.DirListing <> nil, 'Directory listing must be assigned');
  CheckEquals(VALID_COMBINED_LISTING, string(OpenedFile.DirListing.GetDebugDump()), 'Invalid combined listing for directory "' + VfsUtils.MakePath([RootDir, '"']));
  FreeAndNil(OpenedFile);

  OpenedFile := VfsOpenFiles.TOpenedFile.Create(999, RootDir);
  OpenedFile.FillDirListing('*.????');
  Check(OpenedFile.DirListing <> nil, 'Directory listing must be assigned');
  CheckEquals(VALID_COMBINED_MASKED_LISTING, string(OpenedFile.DirListing.GetDebugDump()), 'Invalid combined masked listing for directory "' + VfsUtils.MakePath([RootDir, '"']));
  FreeAndNil(OpenedFile);
  // * * * * * //
