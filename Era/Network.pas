...
function FireRemoteEvent (DestPlayerId: integer; const EventName: myAStr; {n} Data: pointer = nil; DataSize: integer = 0;
...
type
  TStreamStartPacketDebug = packed record
    ...
    EventName:    UtilsB2.TEmptyRec; // array EventNameLen of char
    ...
    PayloadSize:  integer;
    Payload:      UtilsB2.TEmptyRec; // array PayloadSize of byte
  end;
  ...
  {O} Data:             TStrBuilder;
    ...
    constructor Create (const EventName: myAStr; TotalSize, UncompressedSize: integer);
...
function IsValidNetworkEventName (const EventName: myAStr): boolean;
begin
...
constructor TIncomingStream.Create (const EventName: myAStr; TotalSize, UncompressedSize: integer);
begin
...
function CreateIncomingStream (SenderPlayerId, StreamId: integer; const EventName: myAStr; TotalSize, UncompressedSize: integer): {U} TIncomingStream;
var
{Un} SenderStreams: TSingleSenderStreams;
...
begin
  ...
  if SenderStreams = nil then begin
    SenderStreams                        := DataLib.NewObjDict(UtilsB2.OWNS_ITEMS);
...
begin
  StaleStreams := DataLib.NewList(not UtilsB2.OWNS_ITEMS);
  // * * * * * //
  ...
  // * * * * * //
  Legacy.FreeAndNil(StaleStreams);
end;

function FireRemoteEvent (DestPlayerId: integer; const EventName: myAStr; {n} Data: pointer; DataSize: integer; {n} ProgressHandler: TNetworkStreamProgressHandler;
                          {n} ProgressHandlerCustomParam: pointer): boolean;
var
  StreamId:          integer;
  SizeWritten:       integer;
  CompressedPayload: UtilsB2.TArrayOfByte;
  PayloadBuf:        pointer;
  ...
  if NetData.MsgId = NETWORK_MSG_ERA_EVENT_STREAM_START then begin
    ...
    if PayloadSize = TotalSize then begin
      if UncompressedSize >= 0 then begin
        EventData := ZlibUtils.Decompress(UtilsB2.PtrOfs(PacketReader.Buf, PacketReader.Pos), PayloadSize);
        EventMan.GetInstance.Fire(EventName, pointer(EventData), Length(EventData));
      end else begin
        EventMan.GetInstance.Fire(EventName, UtilsB2.PtrOfs(PacketReader.Buf, PacketReader.Pos), PayloadSize);
      end;
    end else begin
...
begin
  ...
  IncomingStreams := DataLib.NewObjDict(UtilsB2.OWNS_ITEMS);
