...
const
  ...
  UNIQUE_SYSTEM_ID_OPT:       myPChar   = Ptr($698838);
  NETWORK_DEF_NAME_OPT:       myPChar   = Ptr($698867);
  ...
  MarkedSavegame: myPChar   = Ptr($68338C);
  Mp3Name:        myPChar   = Ptr($6A33F4);
...
type
  ...
  TExtString = packed record
    ...
    function ToString: myAStr;
  end;

  PH3Str = ^TH3Str;
  TH3Str = packed record
    Special:  integer;
    Value:    myPChar;   // Zero-terminated string or null. If not null, pbyte(Value - 1) is reference counter (zero for single owner) or -1 for constants
    Len:      integer;
    Capacity: integer;
    ...
    procedure AssignPchar ({n} Str: myPChar; StrLen: integer = -1);
  end;
  ...
  TDlgTextLines = packed record
    ...
    procedure Reset;
    procedure AppendLine ({n} Line: myPChar; LineLen: integer = -1);
  end;
  ...
  TSecSkillNames = array [0..MAX_SECONDARY_SKILLS - 1] of myPChar;
  ...
  TSecSkillDesc = packed record
    case boolean of
      ...
      true: (
        Descs: array [0..SKILL_LEVEL_EXPERT - 1] of myPChar;
      );
  ...
  TSecSkillText = packed record
    case boolean of
      ...
      true: (
        Texts: array [0..SKILL_LEVEL_EXPERT] of myPChar;
      );
  ...
  TResourceNamer = class
   ...
   public
    ...
    function  GetResourceName (const OrigResourceName: myAStr): myAStr;

    (* Returns new globally unique resource name. *)
    function  GenerateUniqueResourceName: myAStr;
  end; // .class TResourceNamer

  PChars12 = ^TChars12;
  TChars12 = packed array [0..11] of myChar;
  ...
  TBinaryTreeItem = object
   public
    ...
    procedure SetName (const aName: myAStr);
    function  GetName: myAStr;
    procedure IncRef;
    procedure DecRef;
  ...
  TFontItem = object (TBinaryTreeItem)
   public
    ...
    CharsDataPtr:    UtilsB2.PEndlessByteArr;
  ...
  TPcxItem = object (TBinaryTreeItem)
   public
    ...
    Buffer:       UtilsB2.PEndlessByteArr;
  end; // .object TPcxItem
  {$ALIGN ON}
  ...
  TPcx8ItemStatic = class
    class function Create (const aName: myAStr; aWidth, aHeight: integer): {On} PPcx8Item;
  end;

  TPcx16ItemStatic = class
    (* Create new Pcx16 image with RefCount = 0 and not assigned to any binary tree *)
    class function Create (const aName: myAStr; aWidth, aHeight: integer): {On} PPcx16Item; static;
    class function Create_ (const aName: myAStr; aWidth, aHeight: integer): {On} PPcx16Item; static;

    (* Uses default pcx loading mechanism to load item, RefCount is increased by one *)
    class function Load (const aName: myAStr): {U} PPcx16Item; static;
  end;

  TPcx24ItemStatic = class
    (* Create new Pcx24 image with RefCount = 0 and not assigned to any binary tree *)
    class function Create (const aName: myAStr; aWidth, aHeight: integer): {On} PPcx24Item; static;
  end;
  ...
  TMonInfo = packed record
    ...
    ShortName:    myPChar; // +8  (4242B7)
    DefName:      myPChar; // +C  loading and setting battlefield in 43DA45
    ...
    NumShots:     integer; // +64
    NumCasts:     integer; // +68 - how many times creature can cast
  ...
  TSpell = packed record // size 0x88 = 136
    ...
    SoundFileName:       myPChar;                 // +4
    DefIndex:            integer;                 // +8 used to get DefName
    Flags:               integer;                 // +C
    Name:                myPChar;                 // +10h
    ShortName:           myPChar;                 // +14h
    Level:               integer;                 // +18h
    ...
    Description:         array [0..3] of myPChar; // +78h
  ...
  THero = packed record
    ...
    Name:         array [0..12] of myChar;        // +23  db*D  = imya,0
  ...
  *)
  ...
  TTown = packed record
    Id:            byte;                         // +0 0,1,2,...
    Owner:         myChar;                       // +1 0,...
    BuiltThisTurn: myChar;                       // +2 - uzhe stroili v etot turn (0-net, 1-da, 2-ne nash gorod)
    _u2:           byte;                         // +3 0
    ...
    MagLevel:      myChar;                       // +14h = uroven' magicheskoj gil'dii v gorode (isp. AI dlya postrojki)
    _u15:          byte;
    ...
    MagicHild:     array [0..4] of myChar;       // +BCh = kolvo zaklinanij v urovne gil'dii
  ...
  TTextTableCells = array of UtilsB2.TArrayOfStr;
  ...
  TTextTable = class
   ...
   public
    ...
    function GetItem (Row, Col: integer): myAStr;

    property Items[Row, Col: integer]: myAStr read GetItem; default;
...
function  GetBattleCellStackId (BattleCell: UtilsB2.PEndlessByteArr): integer;
...
function  GetMapFileName: myAStr;
function  GetCampaignFileName: myAStr;
...
procedure ShowMessage (const Mes: myAStr);
function  Ask (const Question: myAStr): boolean;
...
procedure PlaySound (FileName: myAStr); overload;

function ParseTextTable (const TextTable: myAStr): TTextTableCells;
...
function TTextTable.GetItem (Row, Col: integer): myAStr;
var
  RowItems: UtilsB2.TArrayOfStr;
...
begin
  ...
  UtilsB2.CopyMem(DataSize, Data, @NetData.RawData);
...
function TExtString.ToString: myAStr;
begin
  SetLength(result, Self.Len);
  UtilsB2.CopyMem(Self.Len, Self.Value, pointer(result));
end;
...
begin
  fNamesMap := DataLib.NewDict(UtilsB2.OWNS_ITEMS, DataLib.CASE_INSENSITIVE);
end;

function TResourceNamer.GetResourceName (const OrigResourceName: myAStr): myAStr;
var
{U} MapItem: TString;
...
begin
  if length(OrigResourceName) <= sizeof(TChars12) then begin
    ...
    if MapItem = nil then begin
      ...
      MapItem                     := TString.Create(':' + Legacy.IntToStr(fAutoId));
...
begin
  inc(fAutoId);
  {!} Assert(fAutoId <> 0, 'Resource Namer IDs are exhausted');
  result := ':' + Legacy.IntToStr(fAutoId);
end;

procedure TBinaryTreeItem.SetName (const aName: myAStr);
begin
  UtilsB2.SetPcharValue(@Self.Name, aName, sizeof(Self.Name) + 1);
end;

function TBinaryTreeItem.GetName: myAStr;
begin
  result := myPChar(@Self.Name[0]);
end;
...
function TBinaryTreeNode.FindItem (const aName: myAStr; var {out} aItem: PBinaryTreeItem): boolean;
var
...
begin
  ...
  if (aWidth > 0) and (aHeight > 0) then begin
    ...
    Legacy.FillChar(result.Palette16, sizeof(TBinaryTreeItem), #0);
    result.Palette16.RefCount := -1;

    Legacy.FillChar(result.Palette24, sizeof(TBinaryTreeItem), #0);
...
class function TPcx16ItemStatic.Create_ (const aName: myAStr; aWidth, aHeight: integer): {On} PPcx16Item;
begin
  result := nil;
...
function GetVal (BaseAddr: pointer; Offset: integer): PValue; overload;
begin
  result := UtilsB2.PtrOfs(BaseAddr, Offset);
end;
...
begin
  ...
  if IsValidPlayerId(PlayerId) then begin
    result := UtilsB2.PtrOfs(ppointer(GAME_MANAGER)^, $20AD0 + sizeof(TPlayer) * PlayerId);
  end;
end;
...
begin
  result := UtilsB2.PtrOfs(ppointer(COMBAT_MANAGER)^, 21708 + STACK_STRUCT_SIZE * StackId + PropOfs);
end;
...
function GetMapFileName: myAStr;
begin
  result := myPChar(pinteger(GAME_MANAGER)^ + $1F6D9);
end;
...
procedure PlaySound (FileName: myAStr); overload;
begin
  PlaySound(myPChar(FileName));
end;

function ParseTextTable (const TextTable: myAStr): TTextTableCells;
const
  ROW_DELIM : myAStr = #13#10;
  COL_DELIM : myChar = #9;
