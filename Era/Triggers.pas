  ...
  if (Msg = WM_KEYDOWN) or (Msg = WM_SYSKEYDOWN) then begin
    ...
    if wParam = KEY_F11 then begin
      ...
      if (RootDlgId = Heroes.ADVMAP_DLGID) and (Heroes.WndManagerPtr^.CurrentDlg.FocusedItemId = -1) then begin
        ...
        UtilsB2.CopyMem(sizeof(SavedZ), @SavedZ, @Erm.z[1]);
      end else begin
...
{U} ExceptionArgs:    UtilsB2.PEndlessIntArr;
...
begin
  ...
  if (NewStackId >= 0) and (NewStackId < Heroes.NUM_BATTLE_STACKS) then begin
    ...
    ShowMessage('OnBeforeBattleStackTurn: invalid stack ID. Expected 0..41. Got: ' + Legacy.IntToStr(NewStackId));
  end;
...
begin
  ...
  if (NewSide >= 0) and (NewSide <= 1) then begin
    ...
    ShowMessage(myAStr('OnBattleStackObtainsTurn: invalid side, set in event handler. Expected 0..1. Got: ') + Legacy.IntToStr(NewSide));
  end;

  if (NewStackInd >= 0) and (NewStackInd < Heroes.NUM_BATTLE_STACKS_PER_SIDE) then begin
    StackInd := NewStackInd;
  end else begin
    ShowMessage(myAStr('OnBattleStackObtainsTurn: invalid new stack index, set in event handler. Expected: 0..20. Got: = ') + Legacy.IntToStr(NewStackInd));
  end;
...
begin
  ...
  if not IsReadOnlyDlg then begin
    ...
    HeroId      := -1;
    TownId      := -1;
    MonType     := UtilsB2.PEndlessIntArr(ppointer(Context.EBP + $8)^)[pinteger(Context.EBP + $0C)^];
...
begin
  MonsWithRegeneration := DataLib.NewObjDict(UtilsB2.OWNS_ITEMS);
