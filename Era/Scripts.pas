...
uses
  SysUtils, StrUtils, UtilsB2, DlgMes, Core, Log, Files, FilesEx, DataLib,
  GameExt, EventMan, Lua, Legacy;


const
  SCRIPTS_DIR  : myAStr = 'Data\Lua';
  ENTRY_SCRIPT : myAStr = 'init.lua';
...
procedure ExecuteLuaScript (const FilePath: myAStr);
var
  Error:   myAStr;
  ErrCode: integer;
...
begin
  ...
  if luaL_loadfile(L, myPChar(FilePath)) <> 0 then begin
    Core.FatalError(myAStr('Failed to load and compile Lua script.'#13#10) + lua_tostring(L, -1));
  end;
  ...
  if ErrCode = TLUA_ERRMEM then begin
    ...
    if lua_isstring(L, -1) then begin
      Error := myAStr('Lua script error occured.'#13#10) + lua_tostring(L, -1);
    end else begin
