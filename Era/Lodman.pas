...
(***)  interface  (***)
uses
  Windows, SysUtils, Math, UtilsB2, Files, Core, Lists, AssocArrays, TypeWrappers, DataLib, Log, Json,
  StrUtils, ApiJack, GameExt, Heroes, Stores, EventMan, DlgMes, Legacy;

const
  MAX_NUM_LODS = 100;
  DEF_NUM_LODS = 8;
  
  LODREDIR_SAVE_SECTION : myAStr = 'Era.ResourceRedirections';
...
procedure GlobalRedirectFile (const OldFileName, NewFileName: myAStr);
function  FindFileLod (const FileName: myAStr; out LodPath: myAStr): boolean;
function  FileIsInLod (const FileName: myAStr; Lod: Heroes.PLod): boolean;
function  FindRedirection (const FileName: myAStr; var {out} Redirected: myAStr; Flags: TFindRedirectionFlags = []): boolean;
function  GetRedirectedName (const FileName: myAStr; Flags: TFindRedirectionFlags = []): myAStr;
...
const
  GLOBAL_REDIRECTIONS_CONFIG_DIR         = myAStr('Data\Redirections');
  GLOBAL_MISSING_REDIRECTIONS_CONFIG_DIR : myAStr = GLOBAL_REDIRECTIONS_CONFIG_DIR + '\Missing';
  MUSIC_DIR                              : myAStr = 'Mp3';
...
begin
  if not Legacy.FileExists('Data\h3abp_sp.lod') then begin
    UnregisterLod(7);
  end;
  
  if not Legacy.FileExists('Data\h3abp_bm.lod') then begin
    UnregisterLod(6);
  end;

  if not Legacy.FileExists('Data\h3psprit.lod') then begin
    UnregisterLod(5);
  end;
  
  if not Legacy.FileExists('Data\h3pbitma.lod') then begin
    UnregisterLod(4);
  end;
  
  if not Legacy.FileExists('Data\h3ab_spr.lod') then begin
    UnregisterLod(3);
  end;
  
  if not Legacy.FileExists('Data\h3ab_bmp.lod') then begin
    UnregisterLod(2);
  end;
  
  if not Legacy.FileExists('Data\h3sprite.lod') then begin
    UnregisterLod(1);
  end;
  
  if not Legacy.FileExists('Data\h3bitmap.lod') then begin
    UnregisterLod(0);
  end;
...
begin
  ...
  if result then begin
    LodPath := myPChar(integer(Lod) + 8);
  end;
end; // .function FindFileLod

function FileIsInLods (const FileName: myAStr): boolean;
var
  FoundLod: myAStr;
...
procedure LoadGlobalRedirectionConfig (const ConfigDir: myAStr; RedirectOnlyMissing: boolean);
var
{O} Config:             TlkJsonObject;
    ResourceName:       myAStr;
    WillBeRedirected:   boolean;
    ConfigFileContents: myAStr;
    i:                  integer;
  ...
  with Files.Locate(ConfigDir + '\*.json', Files.ONLY_FILES) do begin
    while FindNext do begin
      if Files.ReadFileContents(ConfigDir + '\' + FoundName, ConfigFileContents) then begin
        UtilsB2.CastOrFree(TlkJson.ParseText(ConfigFileContents), TlkJsonObject, Config);

        if Config <> nil then begin
          for i := 0 to Config.Count - 1 do begin
            ResourceName := myAStr(Config.NameOf[i]);

            if GlobalLodRedirs[ResourceName] = nil then begin
              WillBeRedirected := not RedirectOnlyMissing;

              if RedirectOnlyMissing then begin
                if Legacy.AnsiEndsText('.mp3', ResourceName) then begin
                  WillBeRedirected := not Legacy.FileExists(MUSIC_DIR + '\' + ResourceName);
                end else if Legacy.AnsiEndsText('.wav', ResourceName) or Legacy.AnsiEndsText('.smk', ResourceName) or Legacy.AnsiEndsText('.bik', ResourceName) then begin
...
begin
  if FindRedirection(myPPChar(Context.EBP + $8)^, Redirected) then begin
    myPPChar(Context.EBP + $8)^ := myPChar(Redirected);
  end;
...
begin
  ...
  for i := 0 to PostponedMissingMediaRedirs.Count - 1 do begin
    ...
    if Legacy.AnsiEndsText('.wav', ResourceName) then begin
...
begin
  ...
  PostponedMissingMediaRedirs := DataLib.NewStrList(UtilsB2.OWNS_ITEMS, DataLib.CASE_SENSITIVE);
