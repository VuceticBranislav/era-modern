...
type
  ...
  TPngObject  = Vcl.Imaging.PngImage.TPngImage;
  TDict       = DataLib.TDict;
  TRect       = Types.TRect;
...
function LoadImage (const FilePath: myAStr): {n} TGraphic;
...
function LoadImageAsPcx16 (FilePath:  myAStr;      PcxName:   myAStr  = '';
...
function LoadPcxEx (const PcxName: myAStr): {On} Heroes.PPcxItem;

(* Loads given png file from cache or file system and returns TRawImage in the best format wrapped into shared resource *)
function LoadPngResource (const FilePath: myAStr): {On} ResLib.TSharedResource;
...
const
  ...
  META_PLAYER_COLOR : myAStr = 'player_color';

  // Formal background pcx name for composed image
  META_BACK_PCX_NAME : myAStr = 'back_pcx_name';

  // Redirected background pcx name for composed image
  META_REDIRECTED_BACK_PCX_NAME : myAStr = 'redirected_back_pcx_name';
...
{O} DefBattleEffects:    {U} TDict {of effect palette pointer}; // Used for clone/blood lust/petrification battle effects implementation
  ...
  DefFramePngFilePathPrefix: myAStr; // Like "D:\Games\Heroes 3\Data\Defs\"
...
begin
  ...
  case Image.PixelFormat of
    Vcl.Graphics.pf1bit:  result := 1;
    Vcl.Graphics.pf4bit:  result := 4;
    Vcl.Graphics.pf8bit:  result := 8;
    Vcl.Graphics.pf16bit: result := 16;
    Vcl.Graphics.pf24bit: result := 24;
    Vcl.Graphics.pf32bit: result := 32;
  else
    result := 0;
  end;
...
function GetImageTypeByName (const ImgName: myAStr): TImageType;
var
  ImgExt: myAStr;

begin
  ImgExt := Legacy.AnsiLowerCase(Legacy.ExtractFileExt(ImgName));
...
begin
  ...
  if Image = nil then begin
    ...
    if GetImageType(Image) = IMG_BMP then begin
      ...
      result.PixelFormat := Vcl.Graphics.pf24bit;
    end else begin
      result             := TBitmap.Create;
      result.PixelFormat := Vcl.Graphics.pf24bit;
...
function Bmp24ToPcx16 (Image: TBitmap; const Name: myAStr): {O} Heroes.PPcx16Item;
var
    Width:           integer;
    Height:          integer;
...
begin
  ...
  for y := 0 to Height - 1 do begin
    ...
    PcxPixels := UtilsB2.PtrOfs(PcxPixels, -Width * PCX16_BYTES_PER_COLOR + PcxScanlineSize);
  end;
end; // .function Bmp24ToPcx16

function Bmp24ToPcx24 (Image: TBitmap; const Name: myAStr): {O} Heroes.PPcx24Item;
var
    Width:           integer;
    Height:          integer;
{U} BmpPixels:       GraphTypes.PColor24;
...
begin
  ...
  for y := 0 to Height - 1 do begin
    BmpPixels := Image.Scanline[y];
    UtilsB2.CopyMem(PcxScanlineSize, BmpPixels, PcxPixels);
...
function LoadPcxEx (const PcxName: myAStr): {On} Heroes.PPcxItem;
begin
  if Legacy.AnsiLowerCase(Legacy.ExtractFileExt(PcxName)) = '.pcx16' then begin
    result := LoadPcx16(Legacy.ChangeFileExt(PcxName, '.pcx'));
  end else begin
...
function LoadRawImage32NoCaching (const FilePath: myAStr): {On} TRawImage32;
var
  FileContents: myAStr;
...
begin
  ...
  if EraZip.ReadFileContentsFromZipFs(FilePath, FileContents) then begin
    result := Libspng.DecodePng(myPChar(FileContents), Length(FileContents));
  end;
end;
...
function ApplyImageBackgroundFromConfig (ForegroundImage: TRawImage; const ForegroundPngFilePath: myAStr): {On} TRawImage32;
var
     BaseConfigKey:        myAStr;
     ConfigKey:            myAStr;
     BackPcxName:          myAStr;
...
begin
  ...
  result        := nil;
  BaseConfigKey := 'era.png_backs.' + Legacy.AnsiLowerCase(EraZip.ToRelativePathIfPossible(ForegroundPngFilePath, GameExt.GameDir));
  ...
  if BackPcxName <> ConfigKey then begin
    ...
    if BackImage <> nil then begin
      ...
      BackImage.DrawToOpaque32Buf(
        Heroes.a2i(myPChar(Trans.Tr(BaseConfigKey + '.x', []))),
        Heroes.a2i(myPChar(Trans.Tr(BaseConfigKey + '.y', []))),
        0,
        0,
      ...
      result.Meta[META_BACK_PCX_NAME]            := TString.Create(Legacy.ChangeFileExt(BackPcxName, '.pcx'));
...
begin
  ...
  if Image32.HasTransparency then begin
    ...
    UtilsB2.Exchange(Image, Image16);
  end;
  ...
  if UsedComposition then begin
    UtilsB2.Exchange(Image, ComposedImage32);
  end;

  ImageSize := Image.Width * Image.Height * Image.GetPixelSize;
  result    := ResLib.ResMan.AddResource(Image, ImageSize, FilePath); Image := nil;
  // * * * * * //
  Legacy.FreeAndNil(Image);
  Legacy.FreeAndNil(Image16);
  Legacy.FreeAndNil(Image32);
  Legacy.FreeAndNil(Image32Alpha);
  Legacy.FreeAndNil(ComposedImage32);
...
procedure GetDefPngFrameColorizationInfo (const DefName, FrameRelPath: myAStr; var PngColorizationInfo: TPngColorizationInfo);
var
  FrameAltPath: myAStr;
  PlayerColor:  integer;
...
begin
  ...
  if PlayerColor in [Heroes.PLAYER_FIRST..Heroes.PLAYER_LAST] then begin
    ...
    FrameAltPath                         := Legacy.ChangeFileExt(FrameRelPath, '_p' + Legacy.IntToStr(PlayerColor) + '.png');
...
begin
  ...
  PngFileName  := StrLib.Concat([Legacy.IntToStr(GroupInd), '_', Legacy.IntToStr(FrameInd), '.png']);
...
procedure GetPcxPngColorizationInfo (const PcxName: myAStr; var PngColorizationInfo: TPngColorizationInfo);
var
  PcxAltName:  myAStr;
  PlayerColor: integer;
...
begin
  ...
  if PlayerColor in [Heroes.PLAYER_FIRST..Heroes.PLAYER_LAST] then begin
    PngColorizationInfo.ColorizationType := COLORIZATION_SPECIAL_COLORS;
    PngColorizationInfo.PlayerColor      := PlayerColor;
    PcxAltName                           := Legacy.ChangeFileExt(PcxName, '_p' + Legacy.IntToStr(PlayerColor) + '.pcx');
...
function GetPcxPng (PcxName: myAStr): {On} ResLib.TSharedResource; overload;
var
  ColorizationInfo: TPngColorizationInfo;
  PngFilePath:      myAStr;
...
procedure ScanDirPcxPngFiles (const DirPath: myAStr);
begin
  with EraZip.LocateInZipFs(DirPath + '\*', Files.FILES_AND_DIRS) do begin
    while FindNext do begin
      if (FoundName <> '.') and (FoundName <> '..') then begin
        ...
        end else if (FoundRec.Rec.Size > 0) and (Legacy.AnsiLowerCase(Legacy.ExtractFileExt(FoundName)) = '.png') then begin
          PcxPngFileMap[Legacy.ChangeFileExt(FoundName, '.pcx')] := TString.Create(FoundPath);
        end;
      end;
    end; // .while
  end; // .with
...
begin
  ...
  try
    if LastPaletteColors = Palette16Colors then begin
      ...
      Pcx8 := UtilsB2.PtrOfs(Palette16Colors, -integer(@Heroes.PPcx8Item(0).Palette16.Colors));
...
begin
  DefFramesPngFileMap := DataLib.NewDict(not UtilsB2.OWNS_ITEMS, DataLib.CASE_INSENSITIVE);
  PcxPngFileMap       := DataLib.NewDict(UtilsB2.OWNS_ITEMS, DataLib.CASE_INSENSITIVE);
  ColorizedPng        := DataLib.NewDict(not UtilsB2.OWNS_ITEMS, DataLib.CASE_INSENSITIVE);
  DefBattleEffects    := DataLib.NewDict(not UtilsB2.OWNS_ITEMS, DataLib.CASE_INSENSITIVE);
