...
(***)  interface  (***)
uses
  SysUtils, Math, UtilsB2, Alg, DataLib,
  PatchApi, Core, ApiJack, EventMan, GameExt, Erm,
  Heroes, AdvErm, Legacy;
...
type
  ...
  TRecruitMonsDlgSetup = packed record
    ...
    _MinOne1:        integer;
    _MinOne2:        integer;
    ClassName:       array [0..31] of myChar;
  ...
  end; // .record TRecruitMonsDlgSetup
...
begin
  ...
  RecruitMonsDlgOpenEvent.Mem           := DataLib.NewDict(UtilsB2.OWNS_ITEMS, DataLib.CASE_SENSITIVE);
  ...
  Legacy.FreeAndNil(RecruitMonsDlgOpenEvent.Mem);
...
begin
  ...
  result := UtilsB2.Flags(PRecruitMonsDlgSetup(Context.ESI).ObjType).Have(DLG_FLAG_AUTO_UPDATE_ADVMAP);
...
begin
  ...
  result := UtilsB2.Flags(Dlg.ObjType).DontHave(DLG_FLAG_CLOSE_ON_BUY) and (Dlg.MonTypes[1] <> -1);
  if not result then begin
    Context.RetAddr := Ptr($55121B);
  end;
end;
function Command_RecruitDlg (NumParams: integer; Params: POneBasedCmdParams; var Error: myAStr): boolean;
var
  SlotInd:        integer;
  SourceId:       integer;
...
begin
  ...
  if NumParams in [2..5] then begin
    ...
    if result then begin
      ...
      if not result then begin
        Error := Legacy.Format('Invalid slot index: %d. Valid value is 0..3', [SlotInd]);
      end;
    end;
    // $type
    ...
    if result then begin
      ...
      if not result then begin
        Error := Legacy.Format('Invalid monsters type: %d. Valid value is -1..%d', [Params[2].Value.v, Heroes.NumMonstersPtr^ - 1]);
      end else begin
    ...
    if result and (NumParams >= 4) and not Params[4].IsStr then begin
      ...
      if not result then begin
        Error := Legacy.Format('Invalid source ID: %d', [SourceId]);
      end else begin
    ...
    if result and (NumParams >= 3) and not Params[3].IsStr then begin
      ...
      if not result then begin
        Error := Legacy.Format('Invalid monsters number: %d. Valid value is 0..%d', [Params[3].Value.v, Heroes.MAX_MONS_IN_STACK]);
...
function Command_RecruitDlg_ShiftSlots (NumParams: integer; Params: POneBasedCmdParams; var Error: myAStr): boolean;
begin
  if not RecruitMonsDlgOpenEvent.Inited then begin
    result := false;
    Error  := 'Dialog structure is not initialized and cannot be accessed outside of appropriate event handlers';
...
function Command_RecruitDlg_SlotIndex (NumParams: integer; Params: POneBasedCmdParams; var Error: myAStr): boolean;
var
  DlgSlotInd: integer;
begin
  if not RecruitMonsDlgOpenEvent.Inited then begin
  ...
  if not (DlgSlotInd in [0..3]) then begin
    result := false;
    Error  := Legacy.Format('Invalid dialog slot index: %d. Expected 0..3', [DlgSlotInd]);
    exit;
  end;
...
function Command_RecruitDlg_Info (NumParams: integer; Params: POneBasedCmdParams; var Error: myAStr): boolean;
var
  ParamName:  myAStr;
  ParamValue: integer;
...
begin
  ...
  if not Params[1].IsStr then begin
    ...
    if ParamName = 'dlgId' then begin
      ...
      Error  := Legacy.Format('!!RD - unknown parameter name "%s"', [ParamName]);
      result := false;
      exit;
    end; // .else
...
function Command_RecruitDlg_Mem (NumParams: integer; Params: POneBasedCmdParams; var Error: myAStr): boolean;
var
...
begin
  ...
  if Params[1].IsStr then begin
    ...
    AssocVarName := Legacy.IntToStr(Params[1].Value.v);
  end;
...
begin
  ...
  if (TownId <> -1) and Math.InRange(DwellingId, 0, NUM_DWELLINGS_PER_TOWN - 1) then begin
    ...
    Legacy.FillChar(DlgSetup^, sizeof(DlgSetup^), 0);
    DlgSetup.VirtTable       := Ptr($640C80);   // := Ptr(RECRUIT_DLG_VIRT_TABLE^) ?  BYME
    DlgSetup._MinOne1        := -1;
    DlgSetup._MinOne2        := -1;
    DlgSetup.ClassName       := 'Unknown'#0;
    DlgSetup.IsTownScreen    := ord(IsTownScreen);
    //DlgSetup.ObjType       := UtilsB2.IfThen(TownId <> -1, Heroes.OBJTYPE_TOWN, -1);
    DlgSetup._One1           := 1;
...
function Command_RecruitDlg_Open (NumParams: integer; Params: POneBasedCmdParams; var Error: myAStr): boolean;
var
...
begin
  ...
  if (TownId <> -1) and not Math.InRange(TownId, 0, NumTowns - 1) then begin
    result := false;
    Error  := Legacy.Format('Invalid town ID: %d. Expected 0..NUM_TOWNS - 1 or -1', [TownId]);
    exit;
  end;
  ...
  if (DwellingId <> -1) and not Math.InRange(DwellingId, 0, NUM_DWELLINGS_PER_TOWN - 1) then begin
    result := false;
    Error  := Legacy.Format('Invalid dwelling ID: %d. Expected 0..13 or -1', [DwellingId]);
    exit;
  end;
  ...
  if (TargetType <> -1) and not Math.InRange(TargetType, RECRUIT_TARGET_TOWN, RECRUIT_TARGET_CUSTOM) then begin
    result := false;
    Error  := Legacy.Format('Invalid target type: %d. Expected 0..2', [TargetType]);
    exit;
  end;
  ...
  then begin
    result := false;
    Error  := Legacy.Format('Invalid target ID: %d. Expected valid town/hero ID or -1', [TargetId]);
    exit;
  end;
  if NumParams >= 5 then begin
...
function Receiver_RD (Cmd: myChar; NumParams: integer; Dummy: integer; CmdInfo: Erm.PErmSubCmd): integer; cdecl;
var
...
begin
  ...
  Core.p.WriteDataPatch(Ptr($51FB9F), [myAStr('9690')]);
  ...
  Core.p.WriteDataPatch(Ptr($5510B1), [myAStr('9090909090909090909090909090909090909090')]);
