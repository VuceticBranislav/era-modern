    ...
    if IsException then begin
      with SavegameWriteCrashDetective do begin
        Log.Write('Stores', 'SavegameWrite',
          ...
          Legacy.Format('NumSections: %d, Last section: %s. Last data ptr: 0x%x. Last data size: %d. Total sections size: %d. Total bytes written: %d.', [
            NumSections,
            LastSectionName,
            integer(LastDataPtr),
            LastDataSize,
      ...
      with DataLib.IterateDict(WritingStorage) do begin
        while IterNext do begin
          SectionName    := IterKey;
          SectionsReport := SectionsReport + IterKey + ': ' + Legacy.IntToStr((IterValue as StrLib.TStrBuilder).Size) + #13#10;
        end;
      end;
